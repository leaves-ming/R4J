...
- 查询链路已具备 Dense / Sparse 双路并行、RRF 融合、可选 rerank provider 装配与引用式回答
- 评估链路已具备测试集加载、结果落库、汇总指标和检索 Top-K 解耦验证
- 当本地 Elasticsearch/OpenSearch 不可达时，当前实现会记录告警并回退到内存检索存储，以保证开发与测试基线不中断
...
5. 如需只跑回归主链路：
```bash
mvn -q -Dtest=IngestionIntegrationTest,QueryApiContractTest,EvaluationPersistenceIntegrationTest test
```

## Verified Quickstart

以下路径已在本仓库于 **2026-04-08** 通过自动化验证：

1. `POST /api/v1/ingestions` 契约与集成测试
2. `POST /api/v1/queries` 契约、并行检索与 fallback 集成测试
3. `POST /api/v1/evaluations` 契约、持久化与 golden set 回归测试
...
- 关键业务日志已覆盖导入、检索、回答、评估四条链路，并统一输出 `traceId`
